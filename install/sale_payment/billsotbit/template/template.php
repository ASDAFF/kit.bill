...
<html>
<head>
	<title><?=Loc::getMessage('SALE_HPS_BILL_KIT_TITLE')?></title>
...
{
	$path = $params['BILL_BACKGROUND_KIT'];
	if (intval($path) > 0)
	{
	...
	$backgroundStyle = $params['BILL_BACKGROUND_STYLE_KIT'];
...
<div style="margin: 0pt; padding: <?=join('pt ', $margin); ?>pt; width: <?=$width; ?>pt; background: <?=$background; ?>">
	...
	<?if ($params['BILL_HEADER_SHOW_KIT'] == 'Y'):
			...
			<tr>
				<? if ($params["BILL_PATH_TO_LOGO_KIT"]) { ?>
					<td style="padding-right: 5pt; padding-bottom: 5pt; ">
						<? $imgParams = CFile::_GetImgParams($params['BILL_PATH_TO_LOGO_KIT']); ?>
						<? $imgWidth = $imgParams['WIDTH'] * 96 / (intval($params['BILL_LOGO_DPI_KIT']) ?: 96); ?>
						<img src="<?=$imgParams['SRC']; ?>" width="<?=$imgWidth; ?>" />
					</td>
				<? } ?>
				<td>
					<?=$params["SELLER_COMPANY_NAME_KIT"]; ?><br><br><?
					...
					<? if ($params["SELLER_COMPANY_PHONE_KIT"]) { ?>
						<?=Loc::getMessage('SALE_HPS_BILL_KIT_SELLER_COMPANY_PHONE', array('#PHONE#' => $params["SELLER_COMPANY_PHONE_KIT"]));?><br>
					<? } ?>
					<? if ($params["SELLER_COMPANY_EMAIL_KIT"]) { ?>
						<?=Loc::getMessage('SELLER_COMPANY_EMAIL_KIT', array('#EMAIL#' => $params["SELLER_COMPANY_EMAIL_KIT"]));?><br>
					<? } ?>
					<? if ($params["SELLER_COMPANY_SITE_KIT"]) { ?>
						<?=Loc::getMessage('SELLER_COMPANY_SITE_KIT', array('#SITE#' => $params["SELLER_COMPANY_SITE_KIT"]));?><br>
					<? } ?>
					<? if ($params["BILL_ORDER_HEADER_TITLE_KIT"]) { ?>
						<br><?=$params["BILL_ORDER_HEADER_TITLE_KIT"]?><br><br>
					<? } ?>
				</td>
				<td>
					<?=Loc::getMessage('SELLER_COMPANY_HEADER_TITLE_KIT')?>
				</td>
			</tr>
		</table>
		<?
		if ($params["SELLER_COMPANY_BANK_NAME_KIT"])
		{
			...
			if ($params["SELLER_COMPANY_BANK_CITY_KIT"])
			{
				...
				$sellerBankCity = Loc::getMessage('SELLER_COMPANY_CITY_BANK_KIT').' '.$sellerBankCity;
			}
			$sellerBank = sprintf(
				"%s %s",
		...
		<table class="acc" width="100%">
			...
			<tr>
				...
				<td valign="bottom">
					<?=Loc::getMessage('SALE_HPS_BILL_KIT_SELLER_BANK_BIK')?><br>
				</td>
				<td style="border-bottom:0" valign="bottom">
					<?=$params["SELLER_COMPANY_BANK_BIC_KIT"]; ?>
				</td>
			</tr>
			<tr>
				<td colspan="4" style="border-top:0;"><?=Loc::getMessage('SALE_HPS_BILL_KIT_SELLER_BANK_NAME')?></td>
				<td>
					<?=Loc::getMessage('SALE_HPS_BILL_KIT_SELLER_ACC_CORR')?><br>
				</td>
				<td style="border-top:0">
					<?=$params["SELLER_COMPANY_BANK_ACCOUNT_CORR_KIT"]; ?>
				</td>
			</tr>
			<tr>
				<td>
					<? if ($params["SELLER_COMPANY_INN_KIT"]) { ?>
						<?=Loc::getMessage('SALE_HPS_BILL_KIT_INN', array('#INN#' => $params["SELLER_COMPANY_INN_KIT"]));?>
					<? } else { ?>
						&nbsp;
					<? } ?>
				</td>
				<td><? if ($params["SELLER_COMPANY_INN_KIT"]) { echo $params["SELLER_COMPANY_INN_KIT"];}?></td>
				<td>
					<?=Loc::getMessage('SALE_HPS_BILL_KIT_KPP');?>
				</td>
				<td>
					<? if ($params["SELLER_COMPANY_KPP_KIT"]) { echo $params["SELLER_COMPANY_KPP_KIT"];}?>
				</td>
				<td rowspan="2" valign="top">
					<?=Loc::getMessage("SALE_HPS_BILL_KIT_SELLER_ACC"); ?>
				</td>
				<td rowspan="2" valign="top">
					<?=$sellerRs; ?>
				</td>
			...
			<tr>
				<td colspan="4">
					<?=$params["SELLER_COMPANY_NAME_KIT"]; ?><br>
					<?=Loc::getMessage('SALE_HPS_BILL_KIT_SELLER_NAME')?>
				</td>
			</tr>
		</table>
	<?endif;?>
	...
	<table width="100%">
		...
		<?if ($params['BILL_HEADER_KIT']):?>
			<tr>
				...
				<td style="font-size: 2em; font-weight: bold; text-align: center">
					<nobr><?=$params['BILL_HEADER_KIT'];?> <?=Loc::getMessage('SALE_HPS_BILL_KIT_SELLER_TITLE', array('#PAYMENT_NUM#' => $order->getField('ACCOUNT_NUMBER'), '#PAYMENT_DATE#' => EditData($params["PAYMENT_DATE_INSERT"]),'#NUMBER#' => $params['NUMBER']));?>
					</nobr>
				</td>
				<td></td>
			</tr>
		...
		<? if ($params["BILL_ORDER_SUBJECT_KIT"]) { ?>
			<tr>
				<td></td>
				<td><?=$params["BILL_ORDER_SUBJECT_KIT"]; ?></td>
				<td></td>
			</tr>
		<? } ?>
		<? if ($params["PAYMENT_DATE_PAY_BEFORE_KIT"]) { ?>
			<tr>
				<td></td>
				<td>
					<?=Loc::getMessage('SALE_HPS_BILL_KIT_SELLER_DATE_END', array('#PAYMENT_DATE_END#' => ConvertDateTime($params["PAYMENT_DATE_PAY_BEFORE_KIT"], FORMAT_DATE) ?: $params["PAYMENT_DATE_PAY_BEFORE_KIT"]));?>
				</td>
				<td></td>
			</tr>
		<? } ?>
	...
	<table width="100%">
		...
		<tr>
			<td><?=($params["SELLER_COMPANY_NAME_KIT"])?Loc::getMessage('SALE_HPS_BILL_KIT_SALE_NAME'):'' ?></td>
			<td><?=($params["SELLER_COMPANY_NAME_KIT"])?$params["SELLER_COMPANY_NAME_KIT"]:'' ?></td>
		</tr>
		<tr>
			<td>
				<?
				if ($params['BILL_PAYER_SHOW_KIT'] == 'Y'){
                    echo Loc::getMessage('SALE_HPS_BILL_KIT_BUYER_NAME');
				}
				?>
			</td>
			<td>
                    ...
                    if ($params['BILL_PAYER_SHOW_KIT'] == 'Y')
                    {
                        if($PersonType == 1) {
                            if ($params["BUYER_PERSON_COMPANY_NAME_CONTACT_KIT"])
                                echo $params["BUYER_PERSON_COMPANY_NAME_CONTACT_KIT"];
                        }
                        else{
                            if($params["BUYER_PERSON_COMPANY_NAME_KIT"])
                                echo $params["BUYER_PERSON_COMPANY_NAME_KIT"];
                        }
                    }
            ...
            <tr>
                ...
                <td>
                    ...
                    if ($params["BUYER_PERSON_COMPANY_INN_KIT"])
                        echo ','.\Bitrix\Main\Localization\Loc::getMessage("SALE_HPS_BILL_KIT_BUYER_INN",array('#INN#'=>$params["BUYER_PERSON_COMPANY_INN_KIT"]));
                    if ($params["BUYER_PERSON_COMPANY_KPP_KIT"])
                        echo ','.\Bitrix\Main\Localization\Loc::getMessage("SALE_HPS_BILL_KIT_BUYER_KPP",array('#KPP#'=>$params["BUYER_PERSON_COMPANY_KPP_KIT"]));
                    if ($params["BUYER_PERSON_COMPANY_OGRN_KIT"])
                        echo ','.\Bitrix\Main\Localization\Loc::getMessage("SALE_HPS_BILL_KIT_BUYER_OGRN",array('#OGRN#'=>$params["BUYER_PERSON_COMPANY_OGRN_KIT"]));
                    if ($params["BUYER_PERSON_COMPANY_PHONE_KIT"])
                        echo ','.\Bitrix\Main\Localization\Loc::getMessage("SALE_HPS_BILL_KIT_BUYER_PHONE",array('#PHONE#'=>$params["BUYER_PERSON_COMPANY_PHONE_KIT"]));
                    if ($params["BUYER_PERSON_COMPANY_ADDRESS_KIT"])
                        echo ','.\Bitrix\Main\Localization\Loc::getMessage("SALE_HPS_BILL_KIT_BUYER_ADDRESS",array('#ADDRESS#'=>$params["BUYER_PERSON_COMPANY_ADDRESS_KIT"]));
                    if ($params["BUYER_PERSON_COMPANY_FAX_KIT"])
                        echo ','.\Bitrix\Main\Localization\Loc::getMessage("SALE_HPS_BILL_KIT_BUYER_FAX",array('#FAX#'=>$params["BUYER_PERSON_COMPANY_FAX_KIT"]));
	...
	foreach ($columnList as $column)
	{
		...
		{
			$arColsCaption[$column] = $params['BILL_COLUMN_'.$column.'_TITLE_KIT'];
		...
		foreach ($arColsCaption as $columnId => $caption)
		{
			...
			switch ($columnId)
			{
				case 'NUMBER':
					...
					break;
				case 'NAME':
					$data = html_entity_decode(htmlspecialcharsbx($productName));
					break;
				case 'QUANTITY':
				...
				case 'MEASURE':
					...
					break;
				case 'PRICE':
					$data = html_entity_decode(SaleFormatCurrency($basketItem->getPrice(), $basketItem->getCurrency(), true));
					break;
				case 'VAT_RATE':
					$data = roundEx($basketItem->getVatRate() * 100, SALE_VALUE_PRECISION)."%";
					break;
				case 'SUM':
					$data = html_entity_decode(SaleFormatCurrency($basketItemPrice * $basketItem->getQuantity(), $basketItem->getCurrency(), true));
					break;
			}
			if ($data !== null)
	...
	<? if ($params["BILL_COMMENT1_KIT"] || $params["BILL_COMMENT2_KIT"]) { ?>
		...
		<? if ($params["BILL_COMMENT1_KIT"]) { ?>
			<?=nl2br(HTMLToTxt(preg_replace(
				...
				htmlspecialcharsback($params["BILL_COMMENT1_KIT"])
			), '', array(), 0)); ?>
			<br>
			<br>
		<? } ?>
		<? if ($params["BILL_COMMENT2_KIT"]) { ?>
			<?=nl2br(HTMLToTxt(preg_replace(
				array('#</div>\s*<div[^>]*>#i', '#</?div>#i'), array('<br>', '<br>'),
				htmlspecialcharsback($params["BILL_COMMENT2_KIT"])
			), '', array(), 0)); ?>
			<br>
			<br>
		<? } ?>
	...
	<?if ($params['BILL_SIGN_SHOW_KIT'] == 'Y'):?>
		...
		<? if (!$blank) { ?>
			<div style="position: relative; ">
				<?=CFile::ShowImage($params["SELLER_COMPANY_DIR_SIGN_KIT"], 200, 50,'style="position:absolute;top:-20px;left:420px;"'); ?>
			</div>
		<? } ?>
		<div style="position: relative">
			<table class="sign" style="border-spacing:7px 0px;border-collapse:unset;">
				<? if ($params["SELLER_COMPANY_DIRECTOR_POSITION_KIT"]) { ?>
					<tr>
						...
						<td style="width: 200pt; text-align:center;border-bottom:1px solid #000;"><?=$params["SELLER_COMPANY_DIRECTOR_POSITION_KIT"]; ?></td>
						...
						<td style="border-bottom:1px solid #000;text-align:center;">
							<? if ($params["SELLER_COMPANY_DIRECTOR_NAME_KIT"]) { ?>
								<?=$params["SELLER_COMPANY_DIRECTOR_NAME_KIT"]; ?>
							<? } ?>
						</td>
					</tr>
					<tr>
				...
				<? if ($params["SELLER_COMPANY_ACCOUNTANT_POSITION_KIT"]) { ?>
					<tr>
						<td style="width: 150pt; "><?=$params["SELLER_COMPANY_ACCOUNTANT_POSITION_KIT"]; ?></td>
						<td style="width: 160pt; border: 1pt solid #000000; border-width: 0pt 0pt 1pt 0pt; text-align: center; ">
							<? if (!$blank) { ?>
								<?=CFile::ShowImage($params["SELLER_COMPANY_ACC_SIGN_KIT"], 200, 50); ?>
							<? } ?>
						</td>
						<td>
							<? if ($params["SELLER_COMPANY_ACCOUNTANT_NAME_KIT"]) { ?>
								(<?=$params["SELLER_COMPANY_ACCOUNTANT_NAME_KIT"]; ?>)
							<? } ?>
						</td>
					</tr>
				<? } ?>
