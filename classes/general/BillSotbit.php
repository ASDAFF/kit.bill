...
class BillKit
{
    ...
    function OnEmailHandler(&$arFields, &$arTemplate)
    {
        ...
        if(isset($arFields['ORDER_REAL_ID']) && $arFields['ORDER_REAL_ID'] > 0)
        {
            ...
            if(count($paymentCollection) > 0)
            {
                foreach ($paymentCollection as $payment)
                {
                    if (!$payment->isPaid())
                    {
                        ...
                        if ($action['ACTION_FILE'] == 'billkit')
                        {
                            ...
                            $arFile['MODULE_ID'] = 'kit.bill';
                            $fid = \CFile::SaveFile($arFile , "kit.bill");
                ...
                if (!empty($files))
                {
                    $events = unserialize(\COption::GetOptionString('kit.bill', 'EVENTS'));
    ...
    {
        $pdfName = \COption::GetOptionString('kit.bill', 'FILE_NAME');
        if(empty($pdfName))
